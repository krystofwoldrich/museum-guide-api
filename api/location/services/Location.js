...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Location.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Location.query(buildQuery({ model: Location, filters }))
  ...
  count: (params) => {
    ...
    return Location.query(buildQuery({ model: Location, filters: _.pick(filters, 'where') })).count();
  },
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = await Location.forge(params).save(data);
  ...
  remove: async (params) => {
    ...
    await Location.updateRelations(params);
  ...
  search: async (params) => {
    ...
    const associations = Location.associations.map(x => x.alias);
    const searchText = Object.keys(Location._attributes)
      .filter(attribute => attribute !== Location.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['string', 'text'].includes(Location._attributes[attribute].type));

    const searchInt = Object.keys(Location._attributes)
      .filter(attribute => attribute !== Location.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['integer', 'decimal', 'float'].includes(Location._attributes[attribute].type));

    const searchBool = Object.keys(Location._attributes)
      .filter(attribute => attribute !== Location.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['boolean'].includes(Location._attributes[attribute].type));
